...
export type TextCheck = 'ok' | 'weak' | 'mismatch' | 'unverified' | 'nospeech' | 'demo';

/**
 * هل يجوز الحكمُ على التلاوة (والاجتياز بها)؟
 *
 * لا تُجاز تلاوةٌ حتى يتبيّن أن المقروء هو الآية المختارة. فالأزمنةُ وحدها —
 * ولو قِيسَت من صوتٍ حقيقي — يمكن أن تُطابق أيَّ كلامٍ عربيّ («أكل أحمد تفاحة»
 * مثلًا)، لذلك يكون إخفاق السماع الذكي (`textUnavailable`) حالةً غير معتمدة:
 * تُعرض فيها الأزمنة للتشخيص والتدريب، ولا تُفتح بها الآية. أمّا العرض التجريبي
 * فهو استثناءٌ صريح لأنه صوتٌ مولَّد من نصّ الآية نفسه.
 */
export function timingVerdictAllowed(textCheck: TextCheck, _textUnavailable?: boolean): boolean {
  return textCheck === 'ok' || textCheck === 'demo';
}
...
export interface AlignmentResult {
   ...
   *   nospeech   لم يُسمع في التسجيل كلامٌ أصلًا (صمتٌ أو ضجيج) — لا اجتياز
   *   demo       عرضٌ تجريبي
   */
  textCheck: TextCheck;
  /**
   * أخفق السماع الذكي: استمع إلى **كلامٍ بيّن** فلم يُخرج لفظًا عربيًّا واحدًا —
