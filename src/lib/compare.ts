...
export interface CompareOpts {
  ...
  tempo?: Tempo;
  refPace?: number | null;
  /** أخفق السماع الذكي: تُعرض مقارنة الأزمنة للتشخيص ولا تُعتمد للاجتياز */
...
export function compareWithReciter(
  ...
  if (!textOk) {
    // ما سُمع من الألفاظ بعيد عن الآية (أو لم يُسمع/يتحقق بعد) — لا يُجيز التوقيتُ وحده
    ...
    note =
      textCheck === 'nospeech'
        ...
        : textCheck === 'unverified' && opts.textUnavailable
          ? 'لم يتبيّن اللفظ بالسماع الذكي، فهذه مقارنة أزمنة للتشخيص فقط ولا تُعتمد للاجتياز حتى يتبيّن نصّ الآية.'
